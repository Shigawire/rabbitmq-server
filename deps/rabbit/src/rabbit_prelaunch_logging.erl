...
set_log_level(Level) ->
    %% Primary log level.
    ?LOG_DEBUG(
       "Changing primary log level to ~s", [Level],
    ...
    lists:foreach(
      fun({Module, _}) ->
              ?LOG_DEBUG(
                 "Changing '~s' module log level to ~s", [Module, Level],
                 #{domain => ?RMQLOG_DOMAIN_GLOBAL}),
    ...
    lists:foreach(
      fun
          (#{id := Id, filters := Filters}) ->
              ?LOG_DEBUG(
                 "Changing '~s' handler log level to ~s", [Id, Level],
                 #{domain => ?RMQLOG_DOMAIN_GLOBAL}),
              Filters1 = lists:map(
                           fun
...
create_global_handlers_conf(#{outputs := Outputs} = GlobalConfig) ->
    ensure_handlers_conf(Outputs, global, GlobalConfig, #{}).
